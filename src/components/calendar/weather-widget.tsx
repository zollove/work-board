...
export function WeatherWidget() {
  ...
  const fetchWeather = async () => {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        try {
          localStorage.setItem("work_board_weather_cache_v1", JSON.stringify(daily));
        } catch (e) {}
      }
    } catch (err: any) {
      if (typeof window !== "undefined") {
        try {
          const cached = localStorage.getItem("work_board_weather_cache_v1");
          if (cached) {
  ...
  useEffect(() => {
    // Initial sync load from cache for instant 0ms rendering
    if (typeof window !== "undefined") {
      try {
        const cached = localStorage.getItem("work_board_weather_cache_v1");
        if (cached) {
